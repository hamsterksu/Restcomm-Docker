...
RUN echo -n "7.5.0.800" > version.txt
RUN wget -qc https://mobicents.ci.cloudbees.com/view/RestComm/job/RestComm/800/artifact/Restcomm-JBoss-AS7-7.5.0.800.zip -O Restcomm-JBoss-AS7.zip && unzip Restcomm-JBoss-AS7.zip -d /opt/ && mv /opt/Restcomm-JBoss-AS7-*/ ${install_dir} && mv version.txt ${install_dir} && rm Restcomm-JBoss-AS7.zip
